...
class SafeZHoming:
    def __init__(self, config):
        ...
        x_pos, y_pos, u_pos, v_pos = config.getfloatlist("home_xy_position", count=4)
        self.home_x_pos, self.home_y_pos, self.home_x_pos, self.home_y_pos = x_pos, y_pos, u_pos, v_pos
    ...
    def cmd_G28(self, gcmd):
        ...
        need_x, need_y, need_z = [gcmd.get(axis, None) is not None
                                  for axis in "XYZUV"]
        if not need_x and not need_y and not need_z and not need_u and not need_v:
            need_x = need_y = need_z = need_u = need_v = True
	...
	if need_u:
		...
		'u' not in kin_status['homed_axes'] or
                'v' not in kin_status['homed_axes']):
                raise gcmd.error("Must home X, Y, U and V axes first")
            # Move to safe XY homing position
            prevpos = toolhead.get_position()
            toolhead.manual_move([self.home_x_pos, self.home_y_pos, self.home_u_pos, self.home_v_pos], self.speed)
            # Home Z
            g28_gcmd = self.gcode.create_gcode_command("G28", "G28", {'Z': '0'})
            self.prev_G28(g28_gcmd)
            # Perform Z Hop again for pressure-based probes
            if self.z_hop:
                toolhead.manual_move([None, None, self.z_hop, None, None], self.z_hop_speed)
            # Move XY back to previous positions
            if self.move_to_previous:
                toolhead.manual_move(prevpos[:4], self.speed)
